  ...
  return (
    <div className="max-w-md h-[75vh] w-full bg-transparent backdrop-blur-xl px-6 py-13 rounded-3xl shadow-xl text-center border border-cyan-500 transition-all duration-300 hover:scale-[1.01]">
      ...
      <h2 className="text-2xl font-semibold tracking-tight text-white">
        {firstName} {lastName}
        <span className="text-sm text-cyan-400 font-light ml-2">({title})</span>
      </h2>
      ...
      {/* About Section */}
      <p className="mt-4 text-white/80 text-sm leading-relaxed px-2">{about}</p>
  ...
  <div className="mt-6 flex flex-wrap justify-center gap-2">
    {skills.map((skill, i) => {
      ...
      return (
        <span
          key={i}
          className="flex items-center gap-2 px-4 py-1.5 text-xs font-mono text-cyan-300 border border-cyan-500/40 rounded-full bg-cyan-500/10 shadow-md hover:bg-cyan-500/20 transition"
        >
          {matchedSkill?.logo && (
            <img
